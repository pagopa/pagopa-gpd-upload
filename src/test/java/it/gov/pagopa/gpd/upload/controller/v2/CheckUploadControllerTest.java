...
class CheckUploadControllerTest {
    ...
    void getUploadStatus_OK() {
        when(statusServiceMock.getUploadStatusV2(BROKER_ID, FILE_ID, ORG_FISCAL_CODE, ServiceType.GPD)).thenReturn(UploadStatusDTO.builder().fileId(FILE_ID).build());
    ...
    void getUploadReport_withStatus_OK() {
        when(statusServiceMock.getReportV2(BROKER_ID, ORG_FISCAL_CODE, FILE_ID, ServiceType.GPD)).thenReturn(UploadReportDTO.builder().fileId(FILE_ID).build());
    ...
    void getUploadReport_withoutStatusRetrieveBlobReport_OK() {
        ...
        when(blobServiceMock.getReportV2(BROKER_ID, ORG_FISCAL_CODE, FILE_ID, ServiceType.GPD)).thenReturn(UploadReportDTO.builder().fileId(FILE_ID).build());
    ...
    void getFileIdList_shouldReturn200WithBodyAndContinuationHeader() {
        ...
        String url = URI_V2 + "/list?from=2025-09-01&to=2025-09-06&size=100";
    ...
    void getFileIdList_shouldReturn400WhenRangeTooLarge() {
        // 10 days > 7
        String url = URI_V2 + "/list?from=2025-09-01&to=2025-09-10&size=100";
    ...
    void getFileIdList_shouldReturn400WhenSizeOutOfBounds() {
        // size < 100
        String url = URI_V2 + "/list?from=2025-09-01&to=2025-09-06&size=50";
        HttpRequest<?> req = HttpRequest.GET(url).contentType(MediaType.APPLICATION_JSON);
